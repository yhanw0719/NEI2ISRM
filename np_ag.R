################################################################################
# This script spatially allocate NEI 2017 np_ag emissions to ISRM grid cells
################################################################################

## load input data
load(file.path(inputdir,'isrm.RData'))
load(file.path(inputdir,'counties.RData'))
xwalk = read.csv(file.path(xwalkdir,'xwalk_np_ag.csv'))

## read raw data and format
raw1 = read.csv(file.path(inputdir,'nonpoint/np_ag/ag_fertilizer_2017NEI_NONPOINT_20200415_15apr2020_v0.csv'),
                skip=15) 
raw2 = read.csv(file.path(inputdir,'nonpoint/np_ag/ag_livestock_2017NEI_NONPOINT_20200415_17apr2020_nf_v2.csv'),
                skip=17) 
raw = rbind(raw1, raw2) %>%
...
## save output
save(dat, file=file.path(outputdir,'np_ag.RData'))
write.csv(sf::st_drop_geometry(dat), file=file.path(outputdir,'np_ag.csv'), row.names=FALSE, quote=TRUE)
