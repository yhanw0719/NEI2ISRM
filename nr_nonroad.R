################################################################################
# This script spatially allocate NEI 2017 nr_nonroad emissions to ISRM grid cells
################################################################################

## load input data
load(file.path(inputdir,'isrm.RData'))
load(file.path(inputdir,'counties.RData'))
load(file.path(inputdir,'nonroad/nr_nonroad/2017nei_nonroad_all.RData'))
xwalk = read.csv(file.path(xwalkdir,'xwalk_nr_nonroad.csv'))

## data formatting
raw = raw %>% 
      dplyr::filter(poll %in% c('VOC_INV','NOX','NH3','SO2','PM25TOTAL')) %>%
...
toc()
#save(dat, file=file.path(inputdir,'nonroad/nr_nonroad/nonroad_intermediate.RData')) 
#load(file.path(inputdir,'nonroad/nr_nonroad/nonroad_intermediate.RData'))
...
dat = dat %>% mutate(Sector='nr_nonroad',  
                     Height=0, Diam=0, Temp=0, Velocity=0) %>%
              rename('SCC'='scc','ISRM'='isrm',
                     'PM25'='PM25TOTAL','SOx'='SO2','NOx'='NOX','VOC'='VOC_INV') %>%
              dplyr::select(Sector, SCC, ISRM, VOC, NOx, NH3, SOx, PM25, Height, Diam, Temp, Velocity) 

## save output
save(dat, file=file.path(outputdir,'nr_nonroad.RData'))
write.csv(sf::st_drop_geometry(dat), file=file.path(outputdir,'nr_nonroad.csv'), row.names=FALSE, quote=TRUE)
