################################################################################
# This script spatially allocate NEI 2017 pt_airports emissions to ISRM grid cells
################################################################################

## load input data
load(file.path(inputdir,'isrm.RData'))
load(file.path(inputdir,'counties.RData'))
raw = read.csv(file.path(inputdir,'point/pt_airports/airports_SmokeFlatFile_POINT_20200618.csv'),
               skip = 12)
...
## save output
save(dat, file=file.path(outputdir,'pt_airports.RData'))
write.csv(sf::st_drop_geometry(dat), file=file.path(outputdir,'pt_airports.csv'), row.names=FALSE, quote=TRUE)
