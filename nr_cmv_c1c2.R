################################################################################
# This script spatially allocate NEI 2017 nr_cmv_c1c2 emissions to ISRM grid cells
################################################################################

## load input data
load(file.path(inputdir,'isrm.RData'))
load(file.path(inputdir,'counties.RData'))
xwalk = read.csv(file.path(xwalkdir,'xwalk_nr_cmv_c1c2.csv'))
sf::sf_use_s2(FALSE)

## read raw data and format
raw1 = read.csv(file.path(inputdir,'nonroad/nr_cmv_c1c2/c1c2_offshore_2017NEI_NONPOINT_20200415_15apr2020_v0.csv'),
                skip=15)
raw2 = read.csv(file.path(inputdir,'nonroad/nr_cmv_c1c2/c1c2_onshore_2017NEI_NONPOINT_20200415_15apr2020_v0.csv'),
                skip=15)
raw = rbind(raw1, raw2) %>%
...
dat = dat %>% mutate(Sector='nr_cmv_c1c2',  
...
if (useAIS){
  ...
  dat = filter(dat, SCC %in% c(2280002101,2280002102)) ## keep NEI port emissions
  
  ## read AIS underway portion
  raw = read.csv(file.path(inputdir,'nonroad/nr_cmv_c1c2/cmv_c1c2_2017adjust_20200422_12US1_2017_US_annual_22apr2020_v0.csv'),
                 skip=6) %>%
        dplyr::filter(poll %in% c('VOC','NOX','NH3','SO2','PM2_5'),
                      scc %in% c(2280002201,2280002202)) %>%
...
## save output
save(dat, file=file.path(outputdir,'nr_cmv_c1c2.RData'))
write.csv(sf::st_drop_geometry(dat), file=file.path(outputdir,'nr_cmv_c1c2.csv'), row.names=FALSE, quote=TRUE)
