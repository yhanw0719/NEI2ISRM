################################################################################
# This script spatially allocate NEI 2017 nr_rail emissions to ISRM grid cells
################################################################################

## load input data
load(file.path(inputdir,'isrm.RData'))
load(file.path(inputdir,'counties.RData'))
xwalk = read.csv(file.path(xwalkdir,'xwalk_nr_rail.csv'))

## read raw data and format
raw = read.csv(file.path(inputdir,'nonroad/nr_rail/rail_2017NEI_NONPOINT_20200415_15apr2020_v0.csv'),
               skip=15) %>%
...
## save output
save(dat, file=file.path(outputdir,'nr_rail.RData'))
write.csv(sf::st_drop_geometry(dat), file=file.path(outputdir,'nr_rail.csv'), row.names=FALSE, quote=TRUE)
