################################################################################
# This script spatially allocate NEI 2017 np_nonpt emissions to ISRM grid cells
################################################################################

## load input data
load(file.path(inputdir,'isrm.RData'))
load(file.path(inputdir,'counties.RData'))
xwalk = read.csv(file.path(xwalkdir,'xwalk_np_nonpt.csv'))

## read raw data and format
raw = read.csv(file.path(inputdir,'nonpoint/np_nonpt/2017NEI_NONPOINT_20200415_07may2020_nf_v2.csv'),
               skip=21) %>%
...
dat = dat %>% mutate(Sector='np_nonpt',  
...
## save output
save(dat, file=file.path(outputdir,'np_nonpt.RData'))
write.csv(sf::st_drop_geometry(dat), file=file.path(outputdir,'np_nonpt.csv'), row.names=FALSE, quote=TRUE)
