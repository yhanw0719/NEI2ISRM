################################################################################
# This script spatially allocate NEI 2017 pt_nonegu emissions to ISRM grid cells
################################################################################

## load input data
load(file.path(inputdir,'isrm.RData'))
raw = read.csv(file.path(inputdir,'point/pt_nonegu/nonegu_SmokeFlatFile_POINT_20200618.csv'),
               skip = 12)
...
## save output
save(dat, file=file.path(outputdir,'pt_nonegu.RData'))
write.csv(sf::st_drop_geometry(dat), file=file.path(outputdir,'pt_nonegu.csv'), row.names=FALSE, quote=TRUE)
