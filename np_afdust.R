################################################################################
# This script spatially allocate NEI 2017 np_afdust emissions to ISRM grid cells
################################################################################

## load input data
load(file.path(inputdir,'isrm.RData'))
load(file.path(inputdir,'counties.RData'))
xwalk = read.csv(file.path(xwalkdir,'xwalk_np_afdust.csv'))

## read raw data and format
raw = read.csv(file.path(inputdir,'nonpoint/np_afdust/afdust_2017NEI_NONPOINT_20200415_27apr2020_nf_v5.csv'),
               skip=22) %>%
...
dat = dat %>% mutate(Sector='np_afdust',  
...
## save output
save(dat, file=file.path(outputdir,'np_afdust.RData'))
write.csv(sf::st_drop_geometry(dat), file=file.path(outputdir,'np_afdust.csv'), row.names=FALSE, quote=TRUE)
