################################################################################
# This script spatially allocate NEI 2017 np_oilgas emissions to ISRM grid cells
################################################################################

## load input data
load(file.path(inputdir,'isrm.RData'))
load(file.path(inputdir,'counties.RData'))
xwalk = read.csv(file.path(xwalkdir,'xwalk_np_oilgas.csv'))

## read raw data and format
raw = read.csv(file.path(inputdir,'nonpoint/np_oilgas/np_oilgas_2017NEI_NONPOINT_20200507_07may2020_v0.csv'),
               skip=15) %>%
...
dat = dat %>% mutate(Sector='np_oilgas',  
...
## save output
save(dat, file=file.path(outputdir,'np_oilgas.RData'))
write.csv(sf::st_drop_geometry(dat), file=file.path(outputdir,'np_oilgas.csv'), row.names=FALSE, quote=TRUE)
